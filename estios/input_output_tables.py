...
def post_read_io_table_wrapper(
    ...
    func: Callable,
    attr_labels: tuple[str, str, str] = DEFAULT_IO_MODEL_ATTR_LABELS,
    **kwargs,
) -> dict[str, Any]:
    """Return a `dict` of `attr_labels` and relevant `results` from `data`.

    Args:
        data: `DataFrame` or `Series` to pass to `func`.
        func: `Callable` to pass `data` to with `**kwargs`.
        attr_labels: Labels to apply in interating over results from
            `func(data, *kwargs)`.

    Returns:
        A `dict` of results from applying `func` to `data`.
    """
...
class InputOutputTable(ModelDataSourcesHandler):
    ...
    def core_final_demand(self) -> DataFrame:
        """Final Demand from `.full_io_table` via `final_demand_column_names`."""
    ...
    def get_aggregated_io_table(self) -> DataFrame | InputOutputTable:
        """Return io_table from `full_io_table` via `_aggregate_io_table_func`."""
...
class InputOutputCPATable(InputOutputTable):
    ...
    def get_aggregated_io_table(self) -> DataFrame | "InputOutputCPATable":
        """Return aggregated `self.core_io_table`."""
        assert self._aggregate_io_table_func
        return self._aggregate_io_table_func(
            self._aggregated_sectors_dict,
            self.code_io_table,
