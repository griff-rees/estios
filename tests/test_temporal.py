...
class TestInputOutputTimeSeries:
    ...
    def test_pass_1_io_model(self, three_cities_io, caplog) -> None:
        ...
        assert str(time_series) == (
            "1 Spatial Input-Output model from 2017-06-01 to 2017-06-01: 10 sectors, 3 regions"
        )

    def test_pass_dupe_io_model(self, three_cities_io, caplog) -> None:
        """Ensure dupe time points are logged.

        Todo:
            * Enforce uniqueness in future.
        """
        with caplog.at_level(INFO):
            time_series = InterRegionInputOutputTimeSeries(
                io_models=[three_cities_io, three_cities_io]
            )
            assert "Duplicate(s) of {'2017-06-01': 2}" in caplog.messages
        assert len(time_series) == 2
        ...
        assert str(time_series) == (
            "2 Spatial Input-Output models from 2017-06-01 to 2017-06-01: 10 sectors, 3 regions"
        )

    def test_2017_quarters(
        self, quarterly_2017_employment_dates, three_cities, caplog
    ) -> None:
        """Test expanding runs over 2017 quarters.

        Todo:
            * Check if the scaling log needs to be reinstated.
        """
        caplog.set_level(INFO)
        config_dict = {
